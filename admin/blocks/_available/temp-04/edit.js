...
class Edit extends Component {
	...
	editorCssStylePrint() {
		 ...
		 */
		const cstextColors = () => {
			return `div[data-block="${clientId}"] main {
						color: ${textColor};
					}`;
		};
		...
		return (
			<Fragment>
				...
				<main data-block={name} className={className}>
					...
					{ /* Middle */}
					<div className="middle-content__wrapper" style={{maxWidth: `${contentMaxWidth}px`}}>
						<BrandUpload
							onSelect={( media ) => {
								const logo = {
									id: media.id,
						...
						<RichText
							...
							placeholder={__( 'Subtitle goes here...', '@@pkg.textdomain' )}
					...
					<div className="bottom-content__wrapper">
						<RichText
							...
							onChange={( value ) => setAttributes( { networksText: value } )} />
