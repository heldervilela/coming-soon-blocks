...
return function ( $attributes, $content ) {
	...
	<main data-blockid="<?php echo $blockID; ?>"
	      data-block="pixelthrone/comingsoon--temp-11"
	      style="color:<?php echo esc_attr($attributes->bodyTextColor) ?>;">

		<!-- Content -->
		<div class="the-content -body__typography" style="max-width: <?php echo esc_attr( $attributes->contentMaxWidth ) ?>px">
		...
		<div class="the-footer">
			...
			<?php
				if ( ! empty( $attributes->copyrightText ) ) {
					echo '<p class="-copyright__typography">' . Utils\esc_allowed_html( $attributes->copyrightText ) . '</p>';
				}
			?>
			</div>
			<div>
